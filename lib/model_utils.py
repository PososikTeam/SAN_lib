...
def find_tensor_peak_batch(heatmap, radius, downsample, threshold = 0.000001):
    ...
    X = np2variable(torch.arange(-radius, radius+1), heatmap.is_cuda, False).view(1, 1, radius*2+1)
    Y = np2variable(torch.arange(-radius, radius+1), heatmap.is_cuda, False).view(1, radius*2+1, 1)
